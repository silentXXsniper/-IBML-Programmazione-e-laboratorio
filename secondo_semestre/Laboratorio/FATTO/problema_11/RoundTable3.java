...
public class RoundTable3 extends josephus {
  ...
  public RoundTable3( int n ) {             // creazione di una tavola
                                           // con n cavalieri
    num = n;
    jug = 1;
    head = range( 2, n );
  ...
  private RoundTable3( int n, int j, IntSList h, IntSList t ) {
  ...
  public RoundTable3 serveNeighbour() {     // serve il commensale vicino a sinistra:
                                           // il commensale servito lascia la tavola
    if ( num < 2 ) {                       // meno di due commensali
      return this;
    } else if ( head.isNull() ) {          // (*)
      IntSList rev = tail.reverse();
      return new RoundTable3( num-1, jug, rev.cdr(), IntSList.NULL_INTLIST );
    } else {                               // (**)
      return new RoundTable3( num-1, jug, head.cdr(), tail );
    }
  }
  ...
  public RoundTable3 passJug() {            // passa la brocca al commensale vicino
                                           // (a sinistra)
    if ( num < 2 ) {                       // meno di due commensali
      return this;
    } else if ( head.isNull() ) {          // (*)
      IntSList rev = ( tail.cons(jug) ).reverse();
      return new RoundTable3( num, rev.car(), rev.cdr(), IntSList.NULL_INTLIST );
    } else {                               // (**)
      return new RoundTable3( num, head.car(), head.cdr(), tail.cons(jug) );
    }
  }
