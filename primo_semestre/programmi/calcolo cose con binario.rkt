...
;il seguente codice trasforma numeri in + - .
...
(define bit-rep   ; val: stringa "0" oppure "1"
  (lambda (n)     ; n: numero intero
    (if (< (abs n) 2)
        (btd-rep n)  ; Assume che btd-rep sia una funzione definita
        (let ((q (quotient n 3))
              (r (remainder n 3)))
          (cond
            ((= r -2)
             (string-append (bit-rep (- q 1)) (btd-rep +1)))
            ((= r +2)
             (string-append (bit-rep (+ q 1)) (btd-rep -1)))
            (else
             (string-append (bit-rep q) (btd-rep r))))))))
