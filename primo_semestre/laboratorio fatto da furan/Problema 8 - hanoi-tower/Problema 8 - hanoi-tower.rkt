...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |Problema 8 - hanoi-tower|) (read-case-sensitive #t) (teachpacks ((lib "hanoi.ss.txt" "installed-teachpacks"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "hanoi.ss.txt" "installed-teachpacks")) #f)))
...
(define hanoi-disks
  (lambda (n k) ; n=dischi da spostare, k=mossa
    (hanoi-disks-rec n k 1 3 2 0 0 0)
    ))

(define hanoi-disks-rec
  (lambda (n k s t d a b c) ; n=dischi da spostare, k=mossa, s,t,d=posizioni, a,b,c=dischi per posizione
    (let ((x (expt 2 (- n 1))))
      (cond ((= n 0) (list (list s a) (list t b) (list d c)))
            ((= k 0) (list (list s (+ a n)) (list t b) (list d c)))
            ((>= k x)(hanoi-disks-rec (- n 1) (- k x) t s d b a (+ c 1)))
            ((< k x) (hanoi-disks-rec (- n 1) k s d t (+ a 1) c b)))
      )
    ))
...
(define hanoi-picture-rec
  (lambda (n k img tot s t d a b c) ; n=dischi da spostare, k=mossa, img=immagine in costruzione, tot=totale dischi s,t,d=posizioni, a,b,c=dischi per posizione
    (let ((x (expt 2 (- n 1))))
      (cond ((= n 0) img) ; (= k 0)
